...
export default function WebsiteUrlInput({
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    try {
      ...
      const validation = await validateWebsiteUrl(urlToValidate);
      
      if (!validation.isValid) {
        setValidationError(validation.message || 'Invalid URL');
      } else {
        onUrlSubmit(urlToValidate);
      }
    } catch (err) {
      setValidationError('Error validating URL');
  ...
  return (
    <div className="bg-white shadow-lg rounded-lg p-6 mb-8">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-xl font-semibold text-gray-900">Add Website Content</h2>
        <button
          onClick={onCancel}
            ...
            <div className="mb-4">
              <div className="mb-2 flex justify-between text-sm">
                <span className="font-medium text-gray-700">
                  {currentStep || 'Processing website...'}
                </span>
            ...
            <div className="flex items-start space-x-3 p-4 bg-blue-50 rounded-md">
              ...
              <div>
                <p className="text-sm text-blue-700">
                  Processing website content. This may take a moment depending on the website size.
                </p>
              </div>
            </div>
          </>
